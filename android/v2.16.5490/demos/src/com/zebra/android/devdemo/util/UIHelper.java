...
public class UIHelper {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            if (activity != null && loadingDialog != null) {
                try{
                    ...
                    //Ignored since the UI is not present
                }
            }
        },  delayMillis);
    }
    ...
    public void showErrorDialogOnGuiThread(final String errorMessage) {
        if (activity != null) {
            activity.runOnUiThread(() -> new AlertDialog.Builder(activity).setMessage(errorMessage).setTitle("Error").setPositiveButton("OK", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int id) {
                    dialog.dismiss();
