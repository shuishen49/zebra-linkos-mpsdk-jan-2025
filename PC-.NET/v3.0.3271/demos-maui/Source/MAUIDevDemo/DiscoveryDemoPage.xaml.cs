                ...
                await Task.Factory.StartNew(() => {
                    switch (GetSelectedDiscoveryMethod()) {
                        ...
                        case DiscoveryMethod.ZebraUsbDrivers:
                            try {
                                foreach (DiscoveredPrinter printer in connectionMgr.GetZebraUsbDriverPrinters()) {
                        ...
                        case DiscoveryMethod.UsbDirect:
                            try {
                                connectionMgr.GetZebraUsbDirectPrinters(discoveryHandler);
                            } catch (NotImplementedException) {
                                Device.BeginInvokeOnMainThread(async () => {
                        ...
                        case DiscoveryMethod.FindAllBluetoothDevices:
                            try {
                                connectionMgr.FindBluetoothPrinters(discoveryHandler);
                            } catch (NotImplementedException) {
                                Device.BeginInvokeOnMainThread(async () => {
